...
def _emit_or_print(message, signal=None, fallback_color_code=None, is_error=False):
    ...
    if signal:
        ...
        if fallback_color_code:
            color_code_to_use = color_map.get(fallback_color_code.lower(), fallback_color_code)
...
def check_tools_exist(tools_list):
    ...
    if missing_tools:
        ...
        _emit_or_print(f"Expected tools directory: {config.TOOLS_DIR}", is_error=True)
        return False
    return True
...
def get_free_disk_space_gb(path):
    try:
        ...
        _emit_or_print("shutil.disk_usage not available to check disk space.", is_error=True)
        return None
...
def run_command(command, cwd=None, output_signal=None, error_signal=None, known_error_codes=None):
    ...
    _emit_or_print(f">> Running: {command_str}", output_signal, fallback_color_code="green")
...
def create_temp_dir(base_name_of_input_file, output_signal=None, error_signal=None):
    ...
    file_name_part_for_prefix = os.path.splitext(os.path.basename(base_name_of_input_file))[0]
    ...
    if not os.path.exists(temp_base_for_this_file):
        try:
            ...
            _emit_or_print(f"Created base for temp storage: \"{temp_base_for_this_file}\"", output_signal, fallback_color_code="green")
        except OSError as e:
            _emit_or_print(f"ERROR: Failed to create base temporary directory {temp_base_for_this_file}: {e}", error_signal, is_error=True)
            return None
    try:
        temp_dir = tempfile.mkdtemp(prefix=temp_dir_prefix, suffix=temp_dir_suffix, dir=temp_base_for_this_file)
        _emit_or_print(f"Created actual temp folder: \"{temp_dir}\"", output_signal, fallback_color_code="green")
        return temp_dir
    except Exception as e: 
        _emit_or_print(f"ERROR: Failed to create temp directory in {temp_base_for_this_file}: {e}", error_signal, is_error=True)
        return None
...
def move_files(src_dir, dest_dir_base, pattern, output_signal=None, error_signal=None, allow_overwrite=False):
    ...
    try:
                ...
                if os.path.exists(current_dest_file_path):
                    if allow_overwrite:
                        _emit_or_print(f"WARNING: Destination \"{current_dest_file_path}\" exists. Overwriting.",
                                       error_signal, fallback_color_code="yellow")
                        try:
                            if os.path.isdir(current_dest_file_path): shutil.rmtree(current_dest_file_path)
                            else: os.remove(current_dest_file_path)
                        ...
                        dest_filename_base, dest_filename_ext = os.path.splitext(os.path.basename(initial_dest_file_path))
                        count = 1
                        while True:
                            new_filename = f"{dest_filename_base}_{count}{dest_filename_ext}"
                            potential_new_dest_path = os.path.join(dest_file_subdir, new_filename)
        ...
        _emit_or_print(f"ERROR: Preparing to move files: {e_prep}", error_signal, is_error=True)
        return False
        ...
        while retries > 0:
            try:
                ...
                if retries == 0:
                    ...
                    _emit_or_print(f"WARNING: {err_msg}, retrying...", error_signal, fallback_color_code="yellow")
                    time.sleep(0.5) 
            ...
            for bin_file in associated_bins:
                 if os.path.exists(bin_file) and bin_file.lower().startswith(base_name.lower()) and bin_file.lower().endswith(".bin"): 
                    if bin_file not in files_to_delete:
                        ...
                        _emit_or_print(f">> Found associated file for deletion: \"{os.path.basename(bin_file)}\"", output_signal, fallback_color_code="green")
        
        for file_to_delete_path in files_to_delete:
            if not os.path.exists(file_to_delete_path): continue

            _emit_or_print(f">> Attempting to send to Recycle Bin/Trash: \"{os.path.basename(file_to_delete_path)}\"", output_signal, fallback_color_code="green")
            deleted_successfully_to_recycle = False
            if send2trash:
                try:
                    send2trash.send2trash(file_to_delete_path)
                    _emit_or_print(f"Source file \"{os.path.basename(file_to_delete_path)}\" sent to Recycle Bin/Trash.", output_signal)
                    deleted_successfully_to_recycle = True
                except Exception as e_s2t:
                    _emit_or_print(f"WARNING: send2trash failed for \"{file_to_delete_path}\": {e_s2t}. Trying next method.", error_signal, fallback_color_code="yellow")
            
            if not deleted_successfully_to_recycle and os.name == 'nt' and os.path.exists(config.TOOL_RECYCLE): 
                _emit_or_print(f">> Attempting to use recycle.exe for \"{file_to_delete_path}\"", output_signal, fallback_color_code="green")
                recycle_success = run_command([config.TOOL_RECYCLE, '-f', file_to_delete_path], output_signal=output_signal, error_signal=error_signal)
                if recycle_success:
                    _emit_or_print(f"Source file \"{os.path.basename(file_to_delete_path)}\" sent to Recycle Bin via recycle.exe.", output_signal)
                    deleted_successfully_to_recycle = True
                else:
                    _emit_or_print(f"WARNING: recycle.exe failed for \"{file_to_delete_path}\". Trying permanent delete.", error_signal, fallback_color_code="yellow")
            
            if not deleted_successfully_to_recycle:
                _emit_or_print(f"WARNING: Recycle Bin/Trash methods failed or unavailable. Attempting permanent delete for \"{file_to_delete_path}\".", error_signal, fallback_color_code="yellow")
                try:
                    os.remove(file_to_delete_path)
                    _emit_or_print(f"Source file \"{os.path.basename(file_to_delete_path)}\" permanently deleted.", output_signal, fallback_color_code="yellow") 
                except OSError as remove_e:
                    _emit_or_print(f"ERROR: Failed to permanently delete source {file_to_delete_path}: {remove_e}", error_signal, is_error=True)


def extract_archive(archive_path, output_dir, output_signal=None, error_signal=None):
    _emit_or_print(f">> Extracting: \"{os.path.basename(archive_path)}\" to \"{output_dir}\"", output_signal, fallback_color_code="green")
...
def process_file(file_path, conversion_func, format_out, format_out2=None,
                 ...
                 target_format_from_worker=None, stage_reporter=None):
    ...
    if not os.path.exists(final_output_destination_base):
        try:
            ...
            _emit_or_print(f"ERROR: Failed to create final output dir {final_output_destination_base}: {e}.", error_signal, is_error=True)
            return False

    if stage_reporter: stage_reporter("Preparing")
    temp_path_for_this_file = create_temp_dir(file_path, output_signal=output_signal, error_signal=error_signal)
    if temp_path_for_this_file is None: return False

    path_to_process_in_temp = file_path
    if config.COPY_LOCALLY:
        _emit_or_print(f">> Copying \"{file_name_base_with_ext}\" to \"{temp_path_for_this_file}\"", output_signal, fallback_color_code="green")
        try:
            target_copy_path = os.path.join(temp_path_for_this_file, file_name_base_with_ext)
            ...
            _emit_or_print(f"ERROR: Failed to copy \"{file_name_base_with_ext}\" to temp: {e}", error_signal, is_error=True)
    ...
    if stage_reporter: stage_reporter("Converting")
    conversion_args = {
    ...
    _emit_or_print(f"Conversion successful reported by function: {conversion_successful}", fallback_color_code="magenta")
    _emit_or_print(f"Temp directory is: {temp_path_for_this_file}", fallback_color_code="magenta")
    _emit_or_print("You can now inspect the temp directory.", fallback_color_code="magenta")
    input("Press Enter to continue...")
    # *** END OF ADDED PAUSE ***

    if stage_reporter: stage_reporter("Finalizing")
    ...
    if conversion_successful:
        ...
        if effective_format_out: 
            ...
            expected_primary_output_full_path = os.path.join(temp_path_for_this_file, expected_primary_output_filename)
            ...
            if os.path.isfile(expected_primary_output_full_path):
                ...
                _emit_or_print(f"DEBUG_UTIL: Successfully located expected primary output: {expected_primary_output_full_path}", output_signal)
            else:
                _emit_or_print(f"DEBUG_UTIL: Direct check os.path.isfile failed for: {expected_primary_output_full_path}", error_signal, is_error=True)
                all_files_in_temp_root = glob.glob(os.path.join(temp_path_for_this_file, "*"))
                _emit_or_print(f"DEBUG_UTIL: Contents of temp root '{temp_path_for_this_file}': {all_files_in_temp_root}", output_signal)
                
                original_glob_results = glob.glob(os.path.join(temp_path_for_this_file, '**', f"*.{effective_format_out}"), recursive=True)
                _emit_or_print(f"DEBUG_UTIL: Original glob '**/ *.{effective_format_out}' found: {original_glob_results}", output_signal)
            ...
            if conversion_func.__name__ == "extract_archive_to_folder_routine":
                archive_output_folder = os.path.join(final_output_destination_base, name_part) 
                if not os.path.exists(archive_output_folder): os.makedirs(archive_output_folder)
                    ...
                    try:
                        if os.path.exists(d_item):
                            if allow_overwrite:
                                ...
                                _emit_or_print(f"Skipping existing item in destination: {d_item}", error_signal, fallback_color_code="yellow")
                                continue 
...
def print_help(formats_in, format_out, format_out2=None):
    _emit_or_print("CLI Help Text Placeholder - Retain your original help text here.", fallback_color_code="cyan")
    pass
