...
class SettingsDialog(QDialog):
    ...
    def _update_dolphintool_rvz_level_spinbox_state(self, compression_text_not_used): 
        ...
        if selected_compression_data == "none":
            ...
            default_level_for_type = config.DEFAULT_SETTINGS["DOLPHINTOOL_RVZ_COMPRESSION_LEVEL"]
    ...
    def _update_dolphintool_wia_level_spinbox_state(self, compression_text_not_used):
        ...
        if selected_compression_data in ["none", "purge"]:
            ...
            default_level_for_type = config.DEFAULT_SETTINGS["DOLPHINTOOL_WIA_COMPRESSION_LEVEL"] 
    ...
    def load_settings_to_ui(self):
        if self.copy_locally_checkbox: self.copy_locally_checkbox.setChecked(config.COPY_LOCALLY)
        if self.temp_dir_edit: self.temp_dir_edit.setText(config.MAIN_TEMP_DIR)

        if self.chdman_threaded_processors_combo_box:
            if config.CHDMAN_NUM_PROCESSORS_MODE == "auto":
                self._set_combobox_by_data(self.chdman_threaded_processors_combo_box, "auto")
            else: 
                self._set_combobox_by_data(self.chdman_threaded_processors_combo_box, config.CHDMAN_NUM_PROCESSORS_MANUAL)
        
        if self.chdman_cd_hunksize_check_box: self.chdman_cd_hunksize_check_box.setChecked(config.CHDMAN_CD_USE_CUSTOM_HUNKS)
        if self.chdman_cd_hunksize_line_edit: self.chdman_cd_hunksize_line_edit.setText(str(config.CHDMAN_CD_HUNKS))
        if self.chdman_cd_compression_check_box: self.chdman_cd_compression_check_box.setChecked(config.CHDMAN_CD_USE_CUSTOM_COMPRESSION)
        if self.chdman_cd_compression_line_edit: self.chdman_cd_compression_line_edit.setText(config.CHDMAN_CD_COMPRESSION_TYPES)
        
        if self.chdman_dvd_hunksize_check_box: self.chdman_dvd_hunksize_check_box.setChecked(config.CHDMAN_DVD_USE_CUSTOM_HUNKS)
        if self.chdman_dvd_hunksize_line_edit: self.chdman_dvd_hunksize_line_edit.setText(str(config.CHDMAN_DVD_HUNKS))
        if self.chdman_dvd_compression_check_box: self.chdman_dvd_compression_check_box.setChecked(config.CHDMAN_DVD_USE_CUSTOM_COMPRESSION)
        if self.chdman_dvd_compression_line_edit: self.chdman_dvd_compression_line_edit.setText(config.CHDMAN_DVD_COMPRESSION_TYPES)

        if self.chdman_laserdisc_hunksize_check_box: self.chdman_laserdisc_hunksize_check_box.setChecked(config.CHDMAN_LD_USE_CUSTOM_HUNKS)
        if self.chdman_laserdisc_hunksize_line_edit: self.chdman_laserdisc_hunksize_line_edit.setText(str(config.CHDMAN_LD_HUNKS))
        if self.chdman_laserdisc_compression_check_box: self.chdman_laserdisc_compression_check_box.setChecked(config.CHDMAN_LD_USE_CUSTOM_COMPRESSION)
        if self.chdman_laserdisc_compression_line_edit: self.chdman_laserdisc_compression_line_edit.setText(config.CHDMAN_LD_COMPRESSION_TYPES)
        if self.chdman_laserdisc_startframe_check_box: self.chdman_laserdisc_startframe_check_box.setChecked(config.CHDMAN_LD_USE_INPUT_START_FRAME)
        if self.chdman_laserdisc_startframe_line_edit: self.chdman_laserdisc_startframe_line_edit.setText(str(config.CHDMAN_LD_INPUT_START_FRAME or ""))
        if self.chdman_laserdisc_inputframes_check_box: self.chdman_laserdisc_inputframes_check_box.setChecked(config.CHDMAN_LD_USE_INPUT_FRAMES)
        if self.chdman_laserdisc_inputframes_line_edit: self.chdman_laserdisc_inputframes_line_edit.setText(str(config.CHDMAN_LD_INPUT_FRAMES or ""))

        if self.chdman_harddisk_hunksize_check_box: self.chdman_harddisk_hunksize_check_box.setChecked(config.CHDMAN_HD_USE_CUSTOM_HUNKS)
        if self.chdman_harddisk_hunksize_line_edit: self.chdman_harddisk_hunksize_line_edit.setText(str(config.CHDMAN_HD_HUNKS))
        if self.chdman_harddisk_compression_check_box: self.chdman_harddisk_compression_check_box.setChecked(config.CHDMAN_HD_USE_CUSTOM_COMPRESSION)
        if self.chdman_harddisk_compression_line_edit: self.chdman_harddisk_compression_line_edit.setText(config.CHDMAN_HD_COMPRESSION_TYPES)
        if self.chdman_harddisk_sector_check_box: self.chdman_harddisk_sector_check_box.setChecked(config.CHDMAN_HD_USE_SECTOR_SIZE)
        if self.chdman_harddisk_sector_line_edit: self.chdman_harddisk_sector_line_edit.setText(str(config.CHDMAN_HD_SECTOR_SIZE or ""))
        if self.chdman_harddisk_size_check_box: self.chdman_harddisk_size_check_box.setChecked(config.CHDMAN_HD_USE_SIZE)
        if self.chdman_harddisk_size_line_edit: self.chdman_harddisk_size_line_edit.setText(str(config.CHDMAN_HD_SIZE or ""))
        if self.chdman_harddisk_chs_check_box: self.chdman_harddisk_chs_check_box.setChecked(config.CHDMAN_HD_USE_CHS)
        if self.chdman_harddisk_chs_c_line_edit: self.chdman_harddisk_chs_c_line_edit.setText(str(config.CHDMAN_HD_CHS_C or ""))
        if self.chdman_harddisk_chs_h_line_edit: self.chdman_harddisk_chs_h_line_edit.setText(str(config.CHDMAN_HD_CHS_H or ""))
        if self.chdman_harddisk_chs_s_line_edit: self.chdman_harddisk_chs_s_line_edit.setText(str(config.CHDMAN_HD_CHS_S or ""))
        if self.chdman_harddisk_template_check_box: self.chdman_harddisk_template_check_box.setChecked(config.CHDMAN_HD_USE_TEMPLATE)
        if self.chdman_harddisk_template_line_edit: self.chdman_harddisk_template_line_edit.setText(config.CHDMAN_HD_TEMPLATE_PATH or "")

        if self.chdman_raw_hunksize_check_box: self.chdman_raw_hunksize_check_box.setChecked(config.CHDMAN_RAW_USE_CUSTOM_HUNKS)
        if self.chdman_raw_hunksize_line_edit: self.chdman_raw_hunksize_line_edit.setText(str(config.CHDMAN_RAW_HUNKS))
        if self.chdman_raw_compression_check_box: self.chdman_raw_compression_check_box.setChecked(config.CHDMAN_RAW_USE_CUSTOM_COMPRESSION)
        if self.chdman_raw_compression_line_edit: self.chdman_raw_compression_line_edit.setText(config.CHDMAN_RAW_COMPRESSION_TYPES)

        if self.chdman_verify_fix_checkbox: self.chdman_verify_fix_checkbox.setChecked(config.CHDMAN_VERIFY_FIX)

        if self.dolphintool_rvz_blocksize_combo_box: self._set_combobox_by_data(self.dolphintool_rvz_blocksize_combo_box, config.DOLPHINTOOL_RVZ_BLOCKSIZE)
        if self.dolphintool_rvz_compression_combo_box: self._set_combobox_by_data(self.dolphintool_rvz_compression_combo_box, config.DOLPHINTOOL_RVZ_COMPRESSION_TYPE)
        if self.dolphintool_rvz_level_spin_box: self.dolphintool_rvz_level_spin_box.setValue(config.DOLPHINTOOL_RVZ_COMPRESSION_LEVEL)
        
        if self.dolphintool_wia_compression_combo_box: self._set_combobox_by_data(self.dolphintool_wia_compression_combo_box, config.DOLPHINTOOL_WIA_COMPRESSION_TYPE)
        if self.dolphintool_wia_level_spin_box: self.dolphintool_wia_level_spin_box.setValue(config.DOLPHINTOOL_WIA_COMPRESSION_LEVEL)

        if self.dolphintool_gcz_blocksize_combo_box: self._set_combobox_by_data(self.dolphintool_gcz_blocksize_combo_box, config.DOLPHINTOOL_GCZ_BLOCKSIZE)

    def browse_temp_dir(self):
        if not self.temp_dir_edit: return
        current_path = self.temp_dir_edit.text() or config.get_default_temp_dir() 
    ...
    def accept(self):
        if self.copy_locally_checkbox: config.COPY_LOCALLY = self.copy_locally_checkbox.isChecked()
        if self.temp_dir_edit:
            temp_dir_text = self.temp_dir_edit.text().strip()
            config.MAIN_TEMP_DIR = temp_dir_text if temp_dir_text else config.get_default_temp_dir()
            if not os.path.exists(config.MAIN_TEMP_DIR):
                parent_dir = os.path.dirname(config.MAIN_TEMP_DIR)
                if not parent_dir or not os.path.isdir(parent_dir): 
                    ...
                    return 
            elif not os.path.isdir(config.MAIN_TEMP_DIR):
                 QMessageBox.warning(self, "Settings Error", f"Temp Directory path exists but is not a directory: {config.MAIN_TEMP_DIR}")
                 return

        if self.chdman_threaded_processors_combo_box:
            selected_proc_data = self.chdman_threaded_processors_combo_box.currentData()
            if selected_proc_data == "auto":
                config.CHDMAN_NUM_PROCESSORS_MODE = "auto"
                config.CHDMAN_NUM_PROCESSORS_MANUAL = config.DEFAULT_SETTINGS["CHDMAN_NUM_PROCESSORS_MANUAL"] 
            else: 
                config.CHDMAN_NUM_PROCESSORS_MODE = "manual"
                config.CHDMAN_NUM_PROCESSORS_MANUAL = int(selected_proc_data)
        
        if self.chdman_cd_hunksize_check_box: config.CHDMAN_CD_USE_CUSTOM_HUNKS = self.chdman_cd_hunksize_check_box.isChecked()
        config.CHDMAN_CD_HUNKS = self._get_int_from_lineedit(self.chdman_cd_hunksize_line_edit, config.DEFAULT_SETTINGS["CHDMAN_CD_HUNKS"])
        if self.chdman_cd_compression_check_box: config.CHDMAN_CD_USE_CUSTOM_COMPRESSION = self.chdman_cd_compression_check_box.isChecked()
        config.CHDMAN_CD_COMPRESSION_TYPES = self._get_str_from_lineedit(self.chdman_cd_compression_line_edit, config.DEFAULT_SETTINGS["CHDMAN_CD_COMPRESSION_TYPES"])

        if self.chdman_dvd_hunksize_check_box: config.CHDMAN_DVD_USE_CUSTOM_HUNKS = self.chdman_dvd_hunksize_check_box.isChecked()
        config.CHDMAN_DVD_HUNKS = self._get_int_from_lineedit(self.chdman_dvd_hunksize_line_edit, config.DEFAULT_SETTINGS["CHDMAN_DVD_HUNKS"])
        if self.chdman_dvd_compression_check_box: config.CHDMAN_DVD_USE_CUSTOM_COMPRESSION = self.chdman_dvd_compression_check_box.isChecked()
        config.CHDMAN_DVD_COMPRESSION_TYPES = self._get_str_from_lineedit(self.chdman_dvd_compression_line_edit, config.DEFAULT_SETTINGS["CHDMAN_DVD_COMPRESSION_TYPES"])

        if self.chdman_laserdisc_hunksize_check_box: config.CHDMAN_LD_USE_CUSTOM_HUNKS = self.chdman_laserdisc_hunksize_check_box.isChecked()
        config.CHDMAN_LD_HUNKS = self._get_int_from_lineedit(self.chdman_laserdisc_hunksize_line_edit, config.DEFAULT_SETTINGS["CHDMAN_LD_HUNKS"])
        if self.chdman_laserdisc_compression_check_box: config.CHDMAN_LD_USE_CUSTOM_COMPRESSION = self.chdman_laserdisc_compression_check_box.isChecked()
        config.CHDMAN_LD_COMPRESSION_TYPES = self._get_str_from_lineedit(self.chdman_laserdisc_compression_line_edit, config.DEFAULT_SETTINGS["CHDMAN_LD_COMPRESSION_TYPES"])
        if self.chdman_laserdisc_startframe_check_box: config.CHDMAN_LD_USE_INPUT_START_FRAME = self.chdman_laserdisc_startframe_check_box.isChecked()
        config.CHDMAN_LD_INPUT_START_FRAME = self._get_int_from_lineedit(self.chdman_laserdisc_startframe_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)
        if self.chdman_laserdisc_inputframes_check_box: config.CHDMAN_LD_USE_INPUT_FRAMES = self.chdman_laserdisc_inputframes_check_box.isChecked()
        config.CHDMAN_LD_INPUT_FRAMES = self._get_int_from_lineedit(self.chdman_laserdisc_inputframes_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)

        if self.chdman_harddisk_hunksize_check_box: config.CHDMAN_HD_USE_CUSTOM_HUNKS = self.chdman_harddisk_hunksize_check_box.isChecked()
        config.CHDMAN_HD_HUNKS = self._get_int_from_lineedit(self.chdman_harddisk_hunksize_line_edit, config.DEFAULT_SETTINGS["CHDMAN_HD_HUNKS"])
        if self.chdman_harddisk_compression_check_box: config.CHDMAN_HD_USE_CUSTOM_COMPRESSION = self.chdman_harddisk_compression_check_box.isChecked()
        config.CHDMAN_HD_COMPRESSION_TYPES = self._get_str_from_lineedit(self.chdman_harddisk_compression_line_edit, config.DEFAULT_SETTINGS["CHDMAN_HD_COMPRESSION_TYPES"])
        if self.chdman_harddisk_sector_check_box: config.CHDMAN_HD_USE_SECTOR_SIZE = self.chdman_harddisk_sector_check_box.isChecked()
        config.CHDMAN_HD_SECTOR_SIZE = self._get_int_from_lineedit(self.chdman_harddisk_sector_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)
        if self.chdman_harddisk_size_check_box: config.CHDMAN_HD_USE_SIZE = self.chdman_harddisk_size_check_box.isChecked()
        config.CHDMAN_HD_SIZE = self._get_str_from_lineedit(self.chdman_harddisk_size_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True) 
        if self.chdman_harddisk_chs_check_box: config.CHDMAN_HD_USE_CHS = self.chdman_harddisk_chs_check_box.isChecked()
        config.CHDMAN_HD_CHS_C = self._get_int_from_lineedit(self.chdman_harddisk_chs_c_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)
        config.CHDMAN_HD_CHS_H = self._get_int_from_lineedit(self.chdman_harddisk_chs_h_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)
        config.CHDMAN_HD_CHS_S = self._get_int_from_lineedit(self.chdman_harddisk_chs_s_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)
        if self.chdman_harddisk_template_check_box: config.CHDMAN_HD_USE_TEMPLATE = self.chdman_harddisk_template_check_box.isChecked()
        config.CHDMAN_HD_TEMPLATE_PATH = self._get_str_from_lineedit(self.chdman_harddisk_template_line_edit, default_if_empty=None, allow_none_if_empty_and_default_is_none=True)

        if self.chdman_raw_hunksize_check_box: config.CHDMAN_RAW_USE_CUSTOM_HUNKS = self.chdman_raw_hunksize_check_box.isChecked()
        config.CHDMAN_RAW_HUNKS = self._get_int_from_lineedit(self.chdman_raw_hunksize_line_edit, config.DEFAULT_SETTINGS["CHDMAN_RAW_HUNKS"])
        if self.chdman_raw_compression_check_box: config.CHDMAN_RAW_USE_CUSTOM_COMPRESSION = self.chdman_raw_compression_check_box.isChecked()
        config.CHDMAN_RAW_COMPRESSION_TYPES = self._get_str_from_lineedit(self.chdman_raw_compression_line_edit, config.DEFAULT_SETTINGS["CHDMAN_RAW_COMPRESSION_TYPES"])

        if self.chdman_verify_fix_checkbox: config.CHDMAN_VERIFY_FIX = self.chdman_verify_fix_checkbox.isChecked()
        
        if self.dolphintool_rvz_blocksize_combo_box: config.DOLPHINTOOL_RVZ_BLOCKSIZE = self.dolphintool_rvz_blocksize_combo_box.currentData()
        if self.dolphintool_rvz_compression_combo_box: config.DOLPHINTOOL_RVZ_COMPRESSION_TYPE = self.dolphintool_rvz_compression_combo_box.currentData()
        if self.dolphintool_rvz_level_spin_box: config.DOLPHINTOOL_RVZ_COMPRESSION_LEVEL = self.dolphintool_rvz_level_spin_box.value()
        
        if self.dolphintool_wia_compression_combo_box: config.DOLPHINTOOL_WIA_COMPRESSION_TYPE = self.dolphintool_wia_compression_combo_box.currentData()
        if self.dolphintool_wia_level_spin_box: config.DOLPHINTOOL_WIA_COMPRESSION_LEVEL = self.dolphintool_wia_level_spin_box.value()

        if self.dolphintool_gcz_blocksize_combo_box: config.DOLPHINTOOL_GCZ_BLOCKSIZE = self.dolphintool_gcz_blocksize_combo_box.currentData()
