...
try:
    ...
    try:
        ...
        if not app_exists: temp_app = QApplication([])
        QMessageBox.critical(None, "Fatal Error", f"PySide6 is not installed or found for gui_main_window.py: {e}")
    except Exception:
...
class ConverterWindow(QMainWindow):
    def __init__(self):
        ...
        main_ui_file_path = os.path.join(os.path.dirname(__file__), "assets", "qt", "main_window.ui")
        if not os.path.exists(main_ui_file_path):
            QMessageBox.critical(self, "Fatal Error", f"Main UI file not found: {main_ui_file_path}")
        ...
        if not self.ui:
            QMessageBox.critical(self, "Fatal UI Load Error", f"Could not load main_window.ui: {loader.errorString()}")
            if app_instance: app_instance.quit()
            else: sys.exit(-1)
            return
        ...
        if self.progress_group_box: self.progress_group_box.setVisible(False)
        if self.overall_progress_bar: self.overall_progress_bar.setValue(0)
        if self.file_progress_bar: self.file_progress_bar.setValue(0)
        if self.overall_label: self.overall_label.setText("Overall Progress:")
        if self.file_label: self.file_label.setText("Current File:")
        ...
        if self.job_type_combo: self.job_type_combo.currentTextChanged.connect(self._on_job_type_changed)
        if self.media_type_combo: self.media_type_combo.currentTextChanged.connect(self._on_media_type_changed)
        if self.add_files_button: self.add_files_button.clicked.connect(self.add_files)
        if self.add_folder_button: self.add_folder_button.clicked.connect(self.add_folder)
        if self.select_input_types_button: self.select_input_types_button.clicked.connect(self._on_select_input_types_clicked)
        
        if self.select_output_folder_button: self.select_output_folder_button.clicked.connect(self._on_select_output_folder_clicked)
        if self.select_output_type_button: self.select_output_type_button.clicked.connect(self._on_select_output_type_clicked)
        if self.output_same_folder_checkbox: self.output_same_folder_checkbox.toggled.connect(self._on_output_same_folder_toggled)
        if self.delete_input_checkbox: self.delete_input_checkbox.toggled.connect(self._on_delete_input_toggled) 
        
        if self.main_action_button: self.main_action_button.clicked.connect(self.start_conversion)
        if self.toggle_log_button: self.toggle_log_button.toggled.connect(self.toggle_log_visibility)
        if self.clear_log_button: self.clear_log_button.clicked.connect(self.clear_log)
        
        if self.actionSettings: self.actionSettings.triggered.connect(self.open_settings)
        if self.actionExit: self.actionExit.triggered.connect(self.close_application) 

        if self.overall_cancel_button: self.overall_cancel_button.clicked.connect(self._request_conversion_stop)
        if self.file_cancel_button: self.file_cancel_button.clicked.connect(self._request_conversion_stop)
        ...
        if self.statusbar: self.statusbar.showMessage("Ready. Select a job type to begin.")
    ...
    def _request_conversion_stop(self):
        if self.conversion_thread and self.conversion_thread.isRunning():
            self.conversion_thread.request_stop()
            if self.overall_cancel_button: self.overall_cancel_button.setEnabled(False)
            if self.file_cancel_button: self.file_cancel_button.setEnabled(False)
            if self.statusbar: self.statusbar.showMessage("Cancellation requested...")
    ...
    def start_conversion(self):
        ...
        for i, row_data in enumerate(self.table_data):
            if row_data[COL_CHECK]: 
                ...
                if not current_active_input_exts and self.selected_media_type_details: 
                    current_active_input_exts = set(self.selected_media_type_details.get("input_ext", []))
        ...
        if not selected_files_data:
            QMessageBox.warning(self, "No Files", "No files selected for conversion (or none match current input filters).")
            return
        ...
        output_folder = None 
        job_requires_output_folder_ui_section = self.selected_media_type_details.get("requires_output_folder", False)
            ...
            if not os.path.isdir(output_folder):
                if not os.path.exists(output_folder): 
                    try:
                        ...
                        if self.log_output_text: self.log_output_text.append(f"INFO: Created output directory: {output_folder}")
                    except Exception as e:
                        QMessageBox.critical(self, "Output Folder Error", f"Could not create output folder: {output_folder}\nError: {e}")
                        return
                else: 
                    QMessageBox.critical(self, "Output Folder Error", f"Specified output path is not a directory: {output_folder}")
                    return
        ...
        current_overwrite_files = self.overwrite_files_checkbox.isChecked() if self.overwrite_files_checkbox else False
        
        self.set_ui_enabled_for_conversion(False) 

        if self.progress_group_box: self.progress_group_box.setVisible(True)
        
        if self.overall_label: self.overall_label.setText(f"Overall Progress (0/{total_files_to_process} files)")
        if self.overall_progress_bar:
            self.overall_progress_bar.setMaximum(total_files_to_process * N_STAGES_PER_FILE) 
        ...
        if self.overall_cancel_button: self.overall_cancel_button.setEnabled(True)
        if self.file_cancel_button: self.file_cancel_button.setEnabled(True) 

        action_button_text = self.main_action_button.text() if self.main_action_button else "Job"
        if self.statusbar: self.statusbar.showMessage(f"Starting '{action_button_text}' for {total_files_to_process} file(s)...")
        
        if self.log_output_text and not self.log_output_text.isVisible():
            if self.toggle_log_button: self.toggle_log_button.setChecked(True) 
        if self.log_output_text: self.log_output_text.clear()
    ...
    def handle_overall_progress_update(self, current_overall_step, total_overall_steps, phase_description):
        ...
        if self.overall_label:
             self.overall_label.setText(f"Overall: {phase_description} ({current_overall_step}/{total_overall_steps} stages)")
        if self.statusbar:
        ...
        if "Preparing" in current_op_label or "Copying" in current_op_label or \
           (self.file_label and (self.file_label.text() == "Current File: -" or \
            ...
            if self.file_label: self.file_label.setText(f"Current: {current_filename_display}")
            if self.file_progress_bar:
                 self.file_progress_bar.setRange(0,100) 
                 self.file_progress_bar.setValue(0) 
    ...
    def handle_conversion_finished(self, success_count, fail_count):
        ...
        if self.statusbar: self.statusbar.showMessage(status_msg)
        if self.log_output_text: self.log_output_text.append(f"\n<b>{status_msg}</b>") 

        if self.overall_progress_bar:
             self.overall_progress_bar.setValue(self.overall_progress_bar.maximum()) 

        if self.file_label: self.file_label.setText("Finished.")
        if self.file_progress_bar: self.file_progress_bar.setValue(100 if total_attempted > 0 else 0)
    ...
    def set_ui_enabled_for_conversion(self, enabled):
        if self.add_files_button: self.add_files_button.setEnabled(enabled)
        if self.add_folder_button: self.add_folder_button.setEnabled(enabled)
        if self.recursive_checkbox: self.recursive_checkbox.setEnabled(enabled)
        if self.file_table: self.file_table.setEnabled(enabled)
        if self.job_type_combo: self.job_type_combo.setEnabled(enabled)
        if self.output_same_folder_checkbox: self.output_same_folder_checkbox.setEnabled(enabled)
        if self.delete_input_checkbox: self.delete_input_checkbox.setEnabled(enabled)
        if self.overwrite_files_checkbox: self.overwrite_files_checkbox.setEnabled(enabled)
        if self.actionSettings: self.actionSettings.setEnabled(enabled) 
        ...
        if enabled: 
            ...
            if self.media_type_combo: self.media_type_combo.setEnabled(False)
            if self.select_input_types_button: self.select_input_types_button.setEnabled(False)
            if self.select_output_type_button: self.select_output_type_button.setEnabled(False)
            if self.output_folder_group_box: self.output_folder_group_box.setEnabled(False)
            if self.main_action_button: self.main_action_button.setEnabled(False) 
    ...
    def _populate_job_types(self):
        if not self.job_type_combo: return
    ...
    def _on_output_same_folder_toggled(self, checked):
        ...
        if self.select_output_folder_button: self.select_output_folder_button.setVisible(show_custom_output_widgets)
        if self.output_folder_path_display: self.output_folder_path_display.setVisible(show_custom_output_widgets)
    ...
    def _on_delete_input_toggled(self, checked):
        ...
        if self.statusbar: self.statusbar.showMessage(f"Delete input files on success: {'Enabled' if checked else 'Disabled'}")

    @Slot(str)
    def _on_job_type_changed(self, selected_job_name):
        if not self.media_type_combo: return
        ...
        self.update_ui_for_job_selection() 
        if self.statusbar: self.statusbar.showMessage(f"Job type '{selected_job_name}' selected. Now select a media type.")
    ...
    def update_ui_for_job_selection(self):
        ...
        if self.media_type_combo: self.media_type_combo.setEnabled(job_is_selected)
    ...
    def update_ui_for_media_selection(self):
        ...
        if media_is_selected and self.selected_media_type_details: 
            ...
            input_ext_str = ", ".join([f".{ext}" for ext in sorted(list(current_display_input_exts))]) if current_display_input_exts else "Any"
            ...
            if self.selected_output_filter: 
                ...
                output_ext_str = "N/A (Folder/Info)" if not self.selected_media_type_details.get("requires_output_folder", False) else "Folder"
            ...
            requires_output_folder_ui_section = self.selected_media_type_details.get("requires_output_folder", False)

        if self.main_action_button: self.main_action_button.setText(action_text)
        if self.input_file_types_label: self.input_file_types_label.setText(f"Input: {input_ext_str}")
        if self.output_file_types_label: self.output_file_types_label.setText(f"Output: {output_ext_str}")
        
        can_select_input_types = media_is_selected and bool(self.selected_media_type_details.get("input_ext") if self.selected_media_type_details else False)
        if self.select_input_types_button: self.select_input_types_button.setEnabled(can_select_input_types)
        
        if self.select_output_type_button: self.select_output_type_button.setEnabled(can_select_output_type)

        if self.output_folder_group_box: self.output_folder_group_box.setEnabled(requires_output_folder_ui_section)
        
        output_in_same_folder = self.output_same_folder_checkbox.isChecked() if self.output_same_folder_checkbox else True
        show_custom_output_path_widgets = requires_output_folder_ui_section and not output_in_same_folder
        
        if self.select_output_folder_button: self.select_output_folder_button.setVisible(show_custom_output_path_widgets)
        if self.output_folder_path_display: self.output_folder_path_display.setVisible(show_custom_output_path_widgets)

        if not requires_output_folder_ui_section or output_in_same_folder: 
            if self.output_folder_path_display: self.output_folder_path_display.clear() 
    ...
    def _on_select_input_types_clicked(self):
        if not self.selected_media_type_details:
            if self.statusbar: self.statusbar.showMessage("Please select a job and media type first.", 3000)
            return
        
        possible_input_exts = self.selected_media_type_details.get("input_ext", [])
        if not possible_input_exts:
            if self.statusbar: self.statusbar.showMessage("No specific input types to filter for this selection.", 3000)
            return
    ...
    def _on_input_filter_type_toggled(self, checked, extension):
        ...
        if self.input_file_types_label:
            if active_filter_display_list:
                ...
                self.input_file_types_label.setText(f"Input: {', '.join(['.' + ext for ext in all_media_exts]) if all_media_exts else 'Any'}")
            else: 
                self.input_file_types_label.setText("Input: N/A")

        if self.statusbar: self.statusbar.showMessage(f"Input filter updated. Active: {', '.join(active_filter_display_list) if active_filter_display_list else 'None (showing all for media type)'}", 3000)
        self._apply_filter_to_table() 
        self.update_convert_button_state()
    ...
    def _on_select_output_type_clicked(self):
        if not self.selected_media_type_details:
            if self.statusbar: self.statusbar.showMessage("Please select a media type first.", 3000)
            return

        possible_output_exts = self.selected_media_type_details.get("output_ext", [])
        if not isinstance(possible_output_exts, list) or not possible_output_exts:
            if self.statusbar: self.statusbar.showMessage("No selectable output types for this media.", 3000)
            return

        menu = QMenu(self)
        for ext_string in possible_output_exts:
            if not ext_string: continue 
            action = QAction(f".{ext_string}", self)
            action.setCheckable(True) 
            action.setChecked(ext_string == self.selected_output_filter)
            action.triggered.connect(lambda checked_status=False, bound_ext_string=ext_string: self._on_output_filter_type_selected(bound_ext_string))
            menu.addAction(action)
    ...
    def _on_output_filter_type_selected(self, extension):
        ...
        if self.output_file_types_label: self.output_file_types_label.setText(f"Output: .{extension}")
        if self.statusbar: self.statusbar.showMessage(f"Output type set to: .{extension}", 3000)
        self.update_convert_button_state()

    def _apply_filter_to_table(self):
        if not self.file_table: return
        ...
        if not visible_exts_for_current_selection and self.selected_media_type_details:
            visible_exts_for_current_selection = set(self.selected_media_type_details.get("input_ext", []))
    ...
    def _on_select_output_folder_clicked(self):
        if not self.output_folder_path_display: return
    ...
    def update_convert_button_state(self):
        if not self.main_action_button: return
    ...
    def _on_table_remove_selected(self):
        ...
        if removed_count > 0:
            ...
            if self.statusbar: self.statusbar.showMessage(f"{removed_count} item(s) removed. {len(self.table_data)} remaining.")
        self.update_convert_button_state()
    ...
    def open_settings(self):
        ...
        if dialog.exec(): 
            if self.statusbar: self.statusbar.showMessage("Settings updated and saved.")
            if self.delete_input_checkbox: 
                 self.delete_input_checkbox.setChecked(config.DELETE_SOURCE_ON_SUCCESS)
        else:
            if self.statusbar: self.statusbar.showMessage("Settings dialog cancelled.")

    @Slot()
    def add_files(self):
        dialog_filter_name = "All Supported Files"
        current_input_exts_to_use_for_dialog = set(menu_definitions.ALL_VALID_INPUT_EXTENSIONS) 
        ...
        patterns = [f"*.{ext}" for ext in sorted(list(current_input_exts_to_use_for_dialog))]
        ...
        if files:
            self.process_added_paths(files, from_add_files_dialog=True, dialog_filter_exts=current_input_exts_to_use_for_dialog)
    ...
    def clear_input_list(self):
        self.table_data = []
        self.update_table_widget() 
        if self.statusbar: self.statusbar.showMessage("Input list cleared.")
        self.update_convert_button_state()
    ...
    def handle_cell_click(self, row, column):
        ...
        if item_flags & Qt.ItemFlag.ItemIsEnabled: 
            ...
            if self.statusbar: self.statusbar.showMessage("This file type is currently filtered out. Adjust input filters to select.", 3000)

    @Slot(bool)
    def toggle_log_visibility(self, checked):
        if self.log_output_text: self.log_output_text.setVisible(checked)
        if self.clear_log_button: self.clear_log_button.setVisible(checked) 
        if self.toggle_log_button: self.toggle_log_button.setText("Hide Log ▲" if checked else "Show Log ▼")
    ...
    def handle_output_update(self, message):
        if self.log_output_text: self.log_output_text.append(message)

    @Slot(str) 
    def handle_error_update(self, message):
        if self.log_output_text: self.log_output_text.append(f"<font color='red'>{message}</font>")
    ...
    def process_added_paths(self, paths, from_add_files_dialog=False, dialog_filter_exts=None):
        ...
        if from_add_files_dialog and dialog_filter_exts:
            ...
            valid_exts_for_adding = set(self.selected_media_type_details.get("input_ext", []))
        else: 
            valid_exts_for_adding = set(menu_definitions.ALL_VALID_INPUT_EXTENSIONS)
        ...
        for item_path_raw in paths:
            ...
            if os.path.isfile(item_path):
                file_ext_lower = os.path.splitext(item_path)[1].lower().lstrip('.')
                if (not valid_exts_for_adding or file_ext_lower in valid_exts_for_adding) and \
                   item_path not in current_paths_in_table:
                    self.table_data.append([True, item_path, file_ext_lower.upper()]) 
                    newly_added_count += 1
                elif item_path not in current_paths_in_table: 
                     ignored_files_log.append(os.path.basename(item_path) + f" (type '.{file_ext_lower}' not in current add filter)")
        ...
        if ignored_files_log and self.log_output_text:
            self.log_output_text.append(f"<font color='orange'>WARNING: Files ignored during add (type mismatch or duplicate): {', '.join(ignored_files_log)}</font>")

        if newly_added_count > 0:
            self.table_data.sort(key=lambda x: x[COL_PATH]) 
            self.update_table_widget() 
        
        if self.statusbar: self.statusbar.showMessage(f"{len(self.table_data)} file(s) in list. ({newly_added_count} added).")
        self.update_convert_button_state()

    def _scan_folder(self, folder_path, recursive, valid_extensions_for_scan):
        found = []
        norm_folder = os.path.normpath(folder_path)
        if not os.path.isdir(norm_folder): return found
        
        norm_temp_main_dir = os.path.normpath(config.MAIN_TEMP_DIR)
        if norm_folder.startswith(norm_temp_main_dir): 
            if self.log_output_text: self.log_output_text.append(f"<font color='orange'>Skipping scan of temp directory: {norm_folder}</font>")
            return found
    ...
    def update_table_widget(self):
        ...
        for r_idx, r_data in enumerate(self.table_data):
            ...
            chk_item.setCheckState(Qt.CheckState.Checked if chk_state_from_model else Qt.CheckState.Unchecked)
...
def run_gui():
    ...
    icon_path = os.path.join(os.path.dirname(__file__), "assets", "qt", "app_icon.ico") 
    ...
    print(f"DEBUG (gui_main_window): Config SUBPROCESS_TIMEOUT: {config.SUBPROCESS_TIMEOUT}") 
